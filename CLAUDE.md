...
### Windows Platform
- **NRPT (Name Resolution Policy Table) silently strips the port from DNS server addresses.** Verified 2026-04-18 on Windows 11 Home 25H2 (version 10.0.26200) ARM64 in UTM. `cmd/agent/dns_windows.go` configures split-DNS via `Add-DnsClientNrptRule -Namespace .<domain> -NameServers <ip>`. The enrollment config specifies `132.145.232.64:15300` but NRPT only stores `132.145.232.64` (no port). Windows DNS client queries the default port 53, where nothing answers — mesh hostname resolution times out at 2–3s per query. Same class as the Linux bug but a different API limitation. Public DNS (via uplink) still works. Fix direction: either use `netsh dns add forwarder` (test whether it accepts non-53 ports), run a local DNS proxy on the agent, or (simpler) adopt MagicDNS-style port-53-at-mesh-address on the lighthouse. Not fixed in v0.9.8-dev; documented in `spike/sleep-wake-evidence/windows-t4-events.txt`.
- **Agent isn't SCM-compatible.** `sc.exe create hop-agent binPath="... serve" && sc.exe start hop-agent` fails with error 1053 because the binary doesn't call `StartServiceCtrlDispatcher` (`golang.org/x/sys/windows/svc` package). The enrollment path prints the `sc.exe create` command as a hint, then claims `✓ Agent started` — but no service gets created and no process stays running. For the Windows sleep/wake test we ran the agent foreground-blocking via SSH (reliable) rather than as a service.
- **Process detachment is fragile on Windows.** `Start-Process -WindowStyle Hidden -RedirectStandardOutput` and scheduled tasks (`schtasks /run` as SYSTEM HIGHEST) both caused the agent to exit within seconds after detaching. Foreground run (console-attached) works fine — suggesting the agent may depend on a console handle, or something in the detach path kills it. Not isolated; worth investigating before Windows ships as a first-class platform.
