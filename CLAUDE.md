...
### macOS Platform
- **Screen Sharing HP mode first-click fails after `hop-agent restart` because hopssh's utun is not registered as a macOS Network Configuration Service.** avconferenced's HP video pipeline classifies interfaces via `SCDynamicStore` / `SCPreferences`; an unregistered utun lands on a cold path where first video RTP takes ~5 s and the internal RTCP watchdog aborts at ~5 s. Tailscale's `NEPacketTunnelProvider` bypasses this by registering the utun as a Service; we can do the same directly via `SystemConfiguration.framework` from our root daemon — no NetworkExtension bundle required. Empirically: hopssh raw utun 0/3, Tailscale NE 3/3, hopssh utun + manual scutil SC injection 2/2. Fix plan: [`docs/macos-hp-screen-sharing-fix.md`](docs/macos-hp-screen-sharing-fix.md).
