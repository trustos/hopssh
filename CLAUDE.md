...
- **TunMTU 2800 KILLS cellular throughput by 6-7x (fixed v0.10.8).** The `nebulacfg.TunMTU = 2800` constant was originally tuned on WiFi LAN with the comment "optimally fills 2 IP fragments (2×1500), halves the packet count per screen frame vs 1440. Benchmarked at 13.7ms avg vs 16.2ms." That benchmark was correct **for WiFi LAN** — Apple's hardware handles intra-LAN fragmentation efficiently. On cellular it's catastrophic: a 2800-byte Nebula packet fragments at the carrier's ~1500 MTU; a single fragment loss costs the entire packet (TCP retransmits the whole thing); cellular paths have non-trivial loss; throughput collapses. Empirical (2026-04-21, Mac mini ↔ MacBook Pro on Yettel BG cellular, identical direct-P2P path verified): **hopssh downlink jumped from 8 Mbps → 59 Mbps** by lowering MTU 2800 → 1280 (Tailscale/WireGuard's default). Tailscale on the same path: 51 Mbps. Hopssh now **matches/beats** Tailscale on cellular, after underperforming by 6.5x. Lesson: **MTU tuning is path-class-dependent, not universally good.** The "fewer packets per screen frame" optimization assumed lossless underlay — false on cellular. 1280 is the safe default; if a future "MTU per peer" feature lands, WiFi LAN can opt up to 2800 again, but the global default must NOT assume LAN. Re-running performance.md's WiFi-LAN benchmarks with 1280 should also happen — the small-packet penalty on LAN may be negligible in practice (Apple's GSO/TSO would re-coalesce for the wire).
