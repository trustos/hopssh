...
- **Inline packet prioritization — first VPN to ship this** — `sendmsg_x` processes the msghdrX array in order, giving us strict priority at the syscall level for free. 3-lane send queue: interactive (control packets OR data <200B), realtime (data 200-1200B), bulk (data ≥1200B). Classification is two reads (`b[0]&0x0f` Nebula MessageType + `len(b)`) — header is plaintext (passed as AAD to EncryptDanger). Result: ~25% reduction in latency-cost-of-load (33ms→26ms avg under 100 Mbps load). Throughput unchanged. Remaining latency floor is WiFi MAC contention.
