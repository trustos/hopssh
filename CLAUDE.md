...
- **macOS `sendmsg_x`/`recvmsg_x` batch syscalls — spike done** — Pure Go (no CGO needed). `msghdrX` struct verified: 56 bytes, matches XNU layout. `recvmsg_x` receive batching works (64 pkts/syscall). `sendmsg_x` send batching with 500μs timer hurts TCP (63 Mbps vs 154 Mbps) — jitter triggers congestion control. **Shipped: receive-only batching** in `patches/04-batch-udp-darwin.patch`. Send batching needs non-blocking TUN reads or per-peer connected sockets.
