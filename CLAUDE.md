...
### Performance
- **Stale NAT-PMP endpoints accumulate in peer hostmaps when the upstream router reuses external port numbers across hosts (shipped v0.10.27, four-layer fix).** Verified 2026-04-25/26 in production fleet (mini + MBP + Linux all behind same CGNAT). Symptom: 5-12 invalid-cert rejections/sec sustained per host, "Mixed" connectivity flicker on dashboard for stable LAN peer pairs, direct tunnels dying every 30-60 min. Initial diagnosis "cross-network scoping bug" was WRONG — `internal/api/renew.go:266` correctly uses `ListForNetwork` and per-enrollment nodeIDs scope `selfEndpointCache` correctly. Actual root cause: the router's NAT-PMP service reallocates external ports across internal hosts as leases expire (e.g., today Linux maps `external:9266→internal:4242`, an hour later mini ends up with `external:9266→internal:4243`). Lighthouse caches retain old endpoints across the reassignment, peers send packets to a stale `46.10.240.91:9266` that now points at the WRONG host, the receiving Nebula instance has the wrong CA loaded → rejection. **Four-layer fix:** **(L1)** per-endpoint TTL tagging end-to-end — agent sends `selfEndpointLifetimesSec[]` from `inst.portmap.LifetimeSeconds()` in heartbeat (RFC 6886 50% safety margin), server tags each cached `endpointHint` with computed `expiresAt`, `mergePeerEndpoints` filters past-expiry; **(L2)** vendor patch 21 `Control.ReplaceStaticHostMap` provides replace-not-add semantics (existing patch 20's `AddStaticHostMap` only sets the singular `learned` slot via `LearnRemote` — it can't evict `Reported` entries); **(L3)** defensive subnet filter in `cmd/agent/renew.go::injectPeerEndpoints` rejects any peerEndpoint whose vpnAddr falls outside this enrollment's /24 — belt-and-braces against future server-side scoping regressions; **(L4)** vendor patch 22 `Control.SetInboundObserver` + `Control.ProbeEndpoint` — agent fires per-endpoint TestRequest probes via `Interface.sendTo` and observes inbound on every authenticated packet, with the goal of reaping endpoints that show no inbound activity. **L4 reap is DISABLED in shipping code** — see "Layer 4 reap is broken under asymmetric CGNAT" below for why. **Empirical results:** mini's invalid-cert rate dropped 144/min → 7/min (95% reduction). MBP similar. Linux residual ~150/min (peers still send to stale endpoints they cached locally before fix; only Layer 4-correctly-implemented could fully reap those). Architectural lessons: (1) per-endpoint TTL is the convergent industry standard (RFC 6886 lease semantics + ICE STUN keepalives) — Tailscale's continuous-probing is the alternative but ~10× engineering investment; (2) the lighthouse `Reported` cache replaces correctly per `unlockedSetV4` (`vendor/.../remote_list.go:422-437`), but peers' LOCAL HostMap caches don't expire — must either prune actively (Layer 4) or accept passive cleanup via Nebula's connection_manager dead-mark (~30s of zero traffic on the CurrentRemote only); (3) verify "shipped" claims against code AND production telemetry for at least 24h before declaring victory — see Layer 4 incident below.
- **Layer 4 endpoint-probe is DISABLED ENTIRELY in production: probes themselves cause hostmap roam-flap, defeating patches 14+15 (v0.10.28 hotfix).** Discovered 2026-04-26 morning while debugging the prior 24h "screen-share doesn't recover after MBP overnight sleep" report. Investigation revealed MBP did NOT sleep — agent log showed continuous `endpoint-probe stats` lines every 60s through the night. The actual symptom: `Host roamed to new udp ip/port` events firing every 5-10s for the mini peer, alternating between `192.168.23.3:4242` (LAN) and `46.10.240.91:4242` (CGNAT public) — bouncing every probe interval. Each Layer 4 `ProbeEndpoint` to a candidate (lighthouse-cached or learned) endpoint elicits a TestReply via Nebula's standard outside.go handler. The TestReply arrives at the probing host with the peer's CGNAT-translated source IP:port (which differs from the probed destination due to asymmetric CGNAT — same root cause as the reap bug). `handleHostRoaming` sees the new source != CurrentRemote and roams. Patches 14 (RoamingSuppressSeconds=10) and 15 (preferred_ranges-aware suppression) are defeated because (a) more than 2 candidates rotate so the suppression window doesn't catch all, (b) the LAN→CGNAT roam SHOULD be blocked by patch 15 but observation shows it's NOT — likely because the active-probe replies have valid cert/connection state, qualifying them as legitimate roams. **Production effect:** TCP / Screen Sharing breaks under continuous CurrentRemote bouncing — packets in flight to the old remote get lost when the kernel route updates. Verified by 30-ping sustained test post-deploy: zero roams = zero loss; pre-deploy = 12+ roams/min + visible screen-share disruption. **Hotfix v0.10.28**: comment out `go runEndpointProbe(ctx, inst)` in `cmd/agent/main.go::startMeshInstance`. Vendor patches 22 (SetInboundObserver, ProbeEndpoint) remain in place — they're zero-overhead infrastructure when no caller invokes them. Layer 4b (nonce-correlated TestRequest/TestReply) would let us re-enable both probing AND reaping safely, but until then the entire scheme is net-negative. **Architectural lesson**: any agent-driven activity that leverages Nebula's existing test-packet flow MUST account for the fact that Nebula's data-path roam logic treats EVERY authenticated inbound packet (including TestReply) as a roaming candidate. To probe without disturbing roam state, either suppress roams during synthetic probes (would need a vendor patch flag on the test packet) OR use a separate non-Nebula channel for liveness checks (UDP echo on a different port). Don't reuse the data-plane test-packet mechanism for app-layer probing.
- **Layer 4 endpoint-probe reap is DISABLED in production: asymmetric CGNAT breaks the inbound-observation liveness signal (verified 2026-04-26 outage).** Vendor patch 22 added `Control.ProbeEndpoint(vpnAddr, remoteAddr)` (sends a TestRequest via `Interface.sendTo` to a SPECIFIC remote bypassing CurrentRemote) plus `Control.SetInboundObserver(cb)` (Nebula calls cb on every authenticated inbound packet with the source IP:port). The agent in `cmd/agent/endpoint_probe.go` fired probes every 5s per endpoint and reaped any endpoint with no observed inbound in 30s. **The flaw:** under asymmetric CGNAT, the peer's outbound REPLY to a probe egresses via a DIFFERENT external port than the NAT-PMP-mapped INBOUND port we probed. So we probe `46.10.240.91:9266`, the packet correctly reaches the peer (NAT-PMP forwards `external:9266→internal:4242`), the peer replies, the reply egresses via `internal:4242→external:4246` (CGNAT-allocated source port for outbound, NOT the NAT-PMP inbound port), and our observer sees inbound from `:4246` not `:9266`. After 30s with no observed inbound from `:9266` exactly, the reaper drops it — even though `:9266` IS a perfectly valid inbound endpoint. **Production impact:** deployed late 2026-04-25, ran overnight. Mini fired 3076 reaps for MBP HOME alone over 9 hours, progressively starving the hostmap. By 09:16:46 the tunnel collapsed (`Tunnel status: dead` × 2 for `[10.42.1.11]`), screen-share between mini and MBP failed entirely. Sleep was NOT involved (no sleep events on mini that night per `network change detected` log absence — verified). The cert-renewal port-bind warnings at 02:27 + 04:49 (`address already in use`) are a SEPARATE long-standing dormant issue, also documented v0.10.26 — they DID NOT cause the failure on their own but kept the same starved Nebula svc alive. **Hotfix in `cmd/agent/endpoint_probe.go::reapStaleEndpoints`:** changed the reap branch to log `WOULD-REAP (disabled)` and increment `reapsSkipped` instead of calling `ctrl.ReplaceStaticHostMap`. Probes + observer still run as no-ops (zero harm, useful telemetry, may help cellular CGNAT keep flow state warm). **Regression test:** `TestEndpointProbe_AsymmetricCGNAT_NoFalseReap` in `cmd/agent/endpoint_probe_test.go` simulates the scenario and proves the bug — must remain a tripwire for any future re-enable. **The correct long-term fix (Layer 4b, deferred)** is nonce-correlated TestRequest/TestReply: encode a unique nonce in the TestRequest payload, match it in the echoed TestReply, attribute the reply to the SPECIFIC endpoint we sent to (not the source IP:port). That gives a CGNAT-asymmetry-safe per-endpoint liveness signal but requires more vendor patching (~half day). **Architectural lessons:** (1) any "endpoint X is dead because we observed no inbound from X exactly" heuristic is wrong under CGNAT; you must correlate each probe with a per-probe response identifier or accept that you cannot tell live-from-dead per-endpoint. (2) Never deploy a known-broken-but-not-yet-disabled fix at end-of-day — the LAST deployed binary at sleep is the one you wake up to. The kill-switch needs to ship FIRST, the probe-on-it second.
- **Live runtime add/remove of mesh instances (shipped v0.10.34).** Pre-v0.10.34 the local API's `POST /local/connect` and `POST /local/disconnect` returned 501 NotImplemented; the leave path returned `restartRequired=true` whenever a live instance was present; the device-flow + token enrollment paths persisted certs to disk but didn't bring the new mesh up. Net effect: any GUI shell (the new macOS Tauri client and any future desktop client) had to tell the user "restart the agent to use this network", which is unacceptable for a click-and-connect UX. The fix wires `connectFn`/`disconnectFn` callbacks from `runServe` (where `servers`, `mux`, and `renewCtx` are in-scope) into `startLocalAPI`. Connect uses a new `tryStartMeshInstance` (refactored from `startMeshInstance`) which returns errors instead of `log.Fatal`-ing — a runtime connect can fail (bad token, port collision, lighthouse unreachable) without crashing the whole agent. Disconnect calls `inst.close()` (which already releases UDP + utun + DNS via the v0.10.33 fixes) and `servers.shutdownInstance(name)` (new helper) to drop the per-instance HTTP listener. Leave now disconnects FIRST (so the renewal loop can't read just-deleted cert files), THEN removes from the registry, THEN deletes the on-disk subdir. Auto-connect is wired into both enrollment paths (`device-flow/poll` + `enroll/token`) so a successful enrollment immediately brings up the mesh — the user sees the new network connected before the response returns. Test coverage: 6 lifecycle tests in `cmd/agent/local_api_lifecycle_test.go` (connect invokes callback / connect propagates error / connect short-circuits when already-connected / disconnect invokes callback / leave disconnects-before-removing with timestamp ordering check / leave response always reports `restartRequired:false`). **Architectural lesson:** boot-time entry points and runtime entry points have different failure semantics — `log.Fatal` is appropriate at boot (agent legitimately can't proceed), wrong at runtime (one bad enroll shouldn't crash everything). When converting a boot-only function to also serve runtime callers, separate the wrapper (`startMeshInstance` for boot, log.Fatal-ing on err) from the worker (`tryStartMeshInstance` returning err) instead of inserting recover() shims.
- **Cert-reload must close OLD svc BEFORE starting new (shipped v0.10.33).** Verified 2026-04-27 morning in production after MBP overnight sleep: dashboard showed both enrollments healthy, `hop-agent status` reported certs `valid (expires in 17h59m)`, but mini was rejecting EVERY MBP handshake with `Invalid certificate from host… error="certificate is expired"` from `10.42.1.11`. Root cause: v0.10.26's `reloadNebula` did **try-then-swap** — start new svc on the same UDP port the OLD svc was holding, expecting new to succeed first. Nebula's listen socket can't share-port, so `nebula.Main()` always failed with `Failed to open udp listener: unable to bind to socket: address already in use`. The v0.10.26 fallback (`keep old svc + watcher alive; will retry`) left `inst.svc` set to the OLD svc; the retry-with-backoff goroutine's first check (`if inst.svc != nil → "svc already restored"`) fired as a **false positive on the very state it was supposed to recover from**, exiting without retrying. Net effect: every cert renewal cycle left the agent's running Nebula serving an OLD cert from memory, even though `hop-agent status` (which reads from disk) showed the new cert. After 24h the OLD cert hard-expired (Layer B 1h grace exhausted) and the mesh died for that enrollment. Production log signature at 02:27, 04:49, 06:30, 06:46 (every renewal): `[renew home] failed to restart Nebula after cert renewal: ... address already in use ... [renew home] retry-reload skipping attempt 1/10: svc already restored`. Fix in `cmd/agent/renew.go::reloadNebula`: (1) stop watcher, set `inst.svc = nil`, close old svc to release the UDP port BEFORE attempting the new svc; (2) call new helper `waitForUDPPortFree(listenPort, 2s)` that polls `net.ListenUDP` on the port until bindable or deadline; (3) start new svc; (4) on failure, schedule retry — and now `inst.svc == nil` precondition makes scheduleRetryReload's `haveSvc` check correctly distinguish "manual restart already restored" from "we just failed". **Three layers of test coverage** (kept as tripwires so this can never silently regress): (1) `cmd/agent/renew_reload_port_release_test.go` — exercises `waitForUDPPortFree` against real UDP sockets in three scenarios (port free → fast return; port held forever → respects deadline; port released mid-wait → detects within one poll tick). (2) `cmd/agent/renew_reload_invariants_test.go` — five runtime invariant tests against fake `meshService` and injected `startNebulaByModeFn`: I1 close-before-start ordering, I2 svc=nil on start failure (the haveSvc-check precondition), I3 retry skips when svc was restored out-of-band, I4 retry proceeds when svc is nil (the previously-broken branch), I5 end-to-end fail-then-recover integration. (3) `cmd/agent/renew_reload_source_test.go` — static text scan against `renew.go` source asserting that `oldSvc.Close()` appears before `startNebulaByModeFn(`, that `waitForUDPPortFreeFn(` is between them, and that `inst.svc = nil` appears before `scheduleRetryReload(`. The source-scan catches reorder-regressions during code review without needing the bug to manifest in production. Also: `retryReloadBackoff` was converted from a plain slice var to `atomic.Pointer[[]time.Duration]` (with init() seeding the production schedule and `retryReloadBackoffSchedule()` accessor) to give tests proper happens-before edges when overriding the schedule for fast-iteration; without this the race detector flagged any test that mutated the schedule. **Architectural lessons:** (1) The v0.10.26 try-then-swap design optimized for the wrong failure mode — it protected against config-error failures (rare, transient) by accepting port-bind failures (frequent, permanent under same-port reload). For cert renewals specifically, only the cert files change; nebula.yaml is untouched, so config-error failures don't apply. Match the recovery primitive to the actual failure class of the operation. (2) Any retry-loop "skip if already restored" check must distinguish "stale leftover" from "real fix" — usually requires nilling the stale before scheduling, OR a generation counter, never relying on `!= nil` alone. (3) `hop-agent status` reads from disk, the running Nebula uses in-memory cert — those can diverge silently, so cert-validity dashboards alone can't catch this class of bug; only peer-side rejection logs (or end-to-end ICMP) can. (4) When an "occasional" warning shows up in production logs, count occurrences rigorously before deciding it's transient — the original v0.10.26 handoff called this "occasional", but it actually fired on every renewal cycle (6× in 24h on the user's MBP), and the difference matters because "occasional" doesn't get fixed.
- **HTTP keep-alives must be DISABLED on every control-plane HTTP client (shipped v0.10.31).** Verified 2026-04-26 in production: after MBP woke from sleep and the UTM Linux VM's bridge transitioned from iPhone Personal Hotspot subnet (`172.20.10.x`) back to home LAN (`192.168.23.232`), Nebula data-plane recovered in seconds (UDP `RebindUDPServer` + `CloseAllTunnels` from `watchNetworkChanges` fired correctly), but **HTTPS heartbeats to `hopssh.com` continued timing out for ~3 minutes** with `context deadline exceeded` while `curl -I https://hopssh.com/healthz` from the same shell returned HTTP/2 405 instantly. Dashboard reported the node offline; manual `sudo systemctl restart hop-agent` was required. Root cause (validated by reading source): every call site in `cmd/agent/renew.go` (`sendHeartbeat:162`, `renewCert:422`) and `cmd/agent/main.go:529` constructs `&http.Client{}` with no Transport set, which Go resolves to the process-wide `http.DefaultTransport` singleton — a shared per-host idle-conn pool. After a physical network change, kernel sockets in that pool are silently dead but Go retries them anyway; each reuse hangs for the full Timeout (10-30s) before the transport rotates to a fresh dial. With 1-2 stale conns per host, observed recovery: ~3 minutes. **Fix: every control-plane client now sets `Transport: &http.Transport{DisableKeepAlives: true}`.** Each request opens a fresh TCP+TLS connection; nothing pools; network changes can't poison anything. **Cost: ~80-150ms TLS handshake per heartbeat (~0.3-0.5% of the 30s heartbeat period).** Decisive validation on the SAME Linux VM that produced the bug: `iptables -j DROP` blackhole to `hopssh.com:443` for 30s, unblock, measure recovery time. **Pre-fix expectation: 60-180s; post-fix actual: 15s.** Three layers of evidence kept as tripwires: (1) `cmd/agent/renew_keepalive_test.go` source-scans for `DisableKeepAlives:true` 1-for-1 with `&http.Client{}` ctors; (2) `cmd/agent/renew_keepalive_behavior_test.go` exercises the actual constructors against `httptest.Server` with a connCounter that records each request's RemoteAddr — 5 requests must produce 5 unique addrs (no reuse). The negative-control case in the same file proves the test infrastructure DOES detect reuse (default-Transport client: 1 conn for 5 requests); without it, the positive tests would be inconclusive. (3) `scripts/test_keepalive_recovery.sh` is the field-reproduction script. **Architectural lesson:** `http.DefaultTransport` is a process-singleton with a shared pool — a fresh `&http.Client{}` does NOT give you a fresh transport. Any long-running daemon making HTTPS calls across network-change events MUST either (a) hold its own `*http.Transport` and call `CloseIdleConnections()` on network change, or (b) `DisableKeepAlives: true`. We chose (b) because it requires no plumbing into `watchNetworkChanges` and self-heals against ALL physical-network events, including ones the watcher can't see (carrier silent path swap, OS routing-table updates without interface flap, etc.). The clock-skew probe in `clock_check.go:104` already had this — that's why `EnsureClockSane` worked perfectly during the same incident while `sendHeartbeat` hung.
...
- **`iperf3 -u` cannot measure UDP path quality through hopssh — it's an iperf3 control-protocol race, not a hopssh bug.** Verified 2026-04-25 on WiFi LAN AND cellular, with fresh server, every rate (5M / 1M / 100K), every duration (2s / 5s / 60s), -R reverse mode included. Always errors with `unable to read from stream socket: Resource temporarily unavailable` immediately after `Cookie/Target Bitrate` are exchanged. Root cause: iperf3 reads TCP MSS from its control connection (`Control connection MSS 1368` on our 1420-byte TUN MTU; Tailscale's 1280 MTU produces MSS 1240) and uses it as the UDP block size. Then it does a non-blocking `recv()` on the control socket immediately after sending parameters, treating EAGAIN as fatal. Our mesh's tiny-TCP-packet latency loses that race; Tailscale's smaller MTU happens to time the reply faster. Real user UDP traffic (RTP, FaceTime, ICE, Screen Sharing AVConference) has no analogous control-channel race and is unaffected — TCP iperf3 through the same mesh hits 350-435 Mb/s on WiFi LAN, beating Tailscale (286-339 Mb/s). The perf_compare script (`scripts/perf_compare.sh`) intentionally skips iperf3 -u and relies on TCP-ramp shape + concurrent ping for path-quality data. If we ever need UDP loss/jitter numbers, use `nuttcp -u` or a custom Go probe — NOT iperf3.
- **Phase G (HTTPS-distributed self-endpoints, shipped v0.10.22) closes the "cellular peer goes silent at idle" gap.** Verified 2026-04-25 on iPhone Personal Hotspot. Symptom: after 16+ minutes of mesh idle, MBP-cellular ↔ mini home went unreachable; `ping 10.42.1.7` 100% loss; mini's home Nebula appeared "frozen" (no inbound activity, UDP RX-Q backed up). Root cause was NOT a Nebula freeze — it was the architecture: (a) MBP cellular CGNAT closed the idle home-network UDP flow to mini, (b) MBP's UDP-to-lighthouse `advertise_addrs` updates were filtered by carrier so server's lighthouse cache went stale, (c) server stopped distributing 10.42.1.11's endpoint via heartbeat responses → mini home had no way to discover MBP's new CGNAT mapping → silent. Fix: agent now sends its OWN observed endpoints (NAT-PMP public + local interface IPs paired with listen port) in the heartbeat POST body (`selfEndpoints []string`); server caches per-node with 15-min TTL (`RenewHandler.selfEndpointCache sync.Map`), merges with lighthouse data when responding to other peers' heartbeats. Independent of UDP-to-lighthouse propagation — works even when the carrier blocks UDP entirely. **Implementation: `cmd/agent/self_endpoints.go` (~110 LOC) + `cmd/agent/renew.go` heartbeat POST extension; `internal/api/renew.go` body decode + cache + merge in peerEndpoints loop. Pure Go, cross-platform via `net.Interfaces()`.** Architectural lesson: the `peerEndpoints` distribution loop must be UDP-independent end-to-end. Lighthouse-only data sources are fragile on cellular.
- **Server stopped pushing `listenPort` in `/api/renew`; cert-renewal reload is now resilient to start-failure (shipped v0.10.26).** Cascading bug verified 2026-04-25 on multi-enrollment MBP. Symptom: user reported "MBP didn't recover after sleep" while screen-sharing. Investigation found the actual root cause was unrelated to sleep — the home-network mesh had been silently broken since the morning's cert renewal at 15:40, and the consequent watcher-death made sleep recovery look broken as a corollary. **Bug 1 (root cause)**: `internal/api/renew.go:129` hardcoded `listenPort := nebulacfg.ListenPort` (constant 4242) and pushed it on every `/api/renew` response. The agent's `applyNebulaConfigUpdate` blindly applied this to nebula.yaml, silently overwriting the per-enrollment port (`Enrollment.ListenPort`, allocated via `NextAvailableListenPort` at enroll time). On multi-enrollment hosts where the secondary enrollment had been allocated 4243+, this caused both enrollments' yaml to converge on 4242, and the next reload tried to bind a port already held by the other instance: `unable to bind to socket: address already in use`. **Bug 2 (cascade)**: pre-v0.10.26 the hot-restart path in `cmd/agent/renew.go::reloadNebula` called `stopWatcher()` + `oldSvc.Close()` BEFORE attempting the new svc start. On failure: early-exit with `inst.svc=nil`, no retry, watcher gone forever. The home network was permanently dead AND its `watchNetworkChanges` goroutine was killed — so subsequent sleep events had no watcher to detect them. **Bug 3 (cosmetic, related)**: occasional "Refusing to handshake with myself" log spam from packets where the inbound cert matched the agent's own; defensive self-loop filters added. **The fix is six changes ("six fixes for one bug")**: (A) server stops pushing `listenPort` entirely; (B) agent rejects mismatched server-pushed `listenPort` defensively in `applyNebulaConfigUpdate`; (C) `reloadNebula` restructured to try-then-swap (only kill watcher + old svc AFTER new svc starts successfully) plus a retry-with-backoff goroutine (5 s → 30 s → 2 m → 5 m → 10 m → 15 m → 30 m × 4 cap, max 10 attempts) so transient failures self-recover without operator intervention; (D) `watchNetworkChanges` gained `defer recover()`, startup log line, and periodic alive log every 200 ticks (~16 min) for fast post-deploy verification; (E) self-VPN-IP filter in both `injectPeerEndpoints` (renew.go) and `injectCachedPeerEndpoints` (peer_cache.go) to prevent self-loops if an agent's own VPN IP ever appears in its own peerEndpoints; (F) boot-time duplicate-port self-heal in `migrateListenPorts` so any future regression OR manual edit producing duplicate ports recovers on next process start (oldest enrollment by `EnrolledAt` keeps the port; others are reallocated via `NextAvailableListenPort`). **Architectural lesson — multi-axis**: (1) "server stops pushing values it doesn't own" is a generalizable principle — the server has no per-node DB column for listen port (`internal/db/migrations/001_initial.sql:41-61` shows the schema), so it never had ground truth to push. Same anti-pattern as the v0.10.17 MTU-push removal. Audit other server-pushed config fields for similar over-reach. (2) Reload-then-fail-then-die is a class of bug — any operation that tears down state before validating a replacement is fragile; always shadow-validate or roll back. (3) Goroutine death without observability is invisible — every long-lived goroutine should log on entry, on exit, and on periodic alive ticks. The "wait for sleep cycle to test if watcher works" pattern was too slow to catch silent death.
- **Phase G's selfEndpoints must filter to the physical egress interface, not enumerate all interfaces (shipped v0.10.25).** Verified 2026-04-25: active Mac-mini ↔ MBP screen-share sessions experienced periodic ~5-10 s "screen black + logout + auto-reconnect" events at a strict ~13-16 min cadence. Mini's `hop-agent.log` showed `Tunnel status: dead vpnAddrs=[10.42.1.11]` events at 10:09, 10:25, 10:41, 10:55, 11:07, 11:18 — Nebula's connection_manager was dead-marking an active tunnel because the host had MULTIPLE competing source paths (en0 LAN, en1 second LAN, ZeroTier `feth*`, Tailscale `utun*`, Parallels `bridge100`), and the kernel routed individual probe packets via different interfaces, causing some probe responses to be lost. Re-handshake took 5-10 s, AVConference RTCP timeout (5-10 s) tripped, Screen Sharing tore the session down + reconnected. Initial hypothesis was that Nebula's existing `lighthouse.local_allow_list=en0` (set in `ensureP2PConfig`) was failing — but it was working correctly on Nebula's UDP advertise side. The actual leak was **Phase G's HTTPS-side `selfEndpoints`** (`cmd/agent/self_endpoints.go::localUnicastAddrs`) which enumerated ALL up non-loopback interfaces unconditionally, then served them via `peerEndpoints` in heartbeat responses, bypassing Nebula's local_allow_list filter entirely. Fix in v0.10.25: `selfEndpoints` now resolves the physical interface via `nebulacfg.DetectPhysicalInterface(extractHost(inst.endpoint()))` and restricts to that interface only via new `localUnicastAddrsOnInterface(name)` helper. Combined with v0.10.25's defensive `tunnels.connection_alive_interval=10s` + `tunnels.pending_deletion_interval=30s` (Nebula defaults: 5 + 10) which absorb transient probe loss without affecting genuine network-change recovery (handled separately by `watchNetworkChanges`). **Architectural lesson: any path-distribution mechanism that BYPASSES Nebula's own filters must apply equivalent filters itself, OR the Nebula filter is incomplete protection.** Cross-platform — pure stdlib `net.Interfaces()` + existing `DetectPhysicalInterface`. Industry-standard behavior — neither Tailscale nor ZeroTier rides through other VPN tap interfaces; each operates over OS physical egress only.
- **A1+A2 (peer-endpoint cache + UDP-hello probe, shipped v0.10.20) closed the cellular cold-start gap to Tailscale.** Empirical 3-pass cellular comparison on 2026-04-25 (Mac mini ↔ MBP, Yettel BG iPhone hotspot, fresh-restart cold start each pass): hopssh peak TCP 40.7/46.0/50.2 Mb/s vs Tailscale 48.0/40.9/57.6 Mb/s — **mean 45.6 vs 48.8 Mb/s, parity within 7%** (vs ~30% gap pre-A1 where hopssh sat at ~30 Mb/s). The s05 cwnd-collapse symptom (pre-A1: 87.6→**22.0**→26.1→51.6 Mb/s) is gone — A1 ship gate Tput@5s/Tput@30s = 233%/183%/127% across 3 passes; ramps either flat or rising. Direct evidence in agent logs: zero `(RELAYED)` lines, first handshake from mini arrives **direct from `46.10.240.91:4242`** in all 3 cold starts (vs pre-A1 where lighthouse-relayed `132.145.232.64:42001 (RELAYED)` won the race). Jitter under TCP load (ping @100ms cadence, ~590 samples/pass): **hopssh wins on cellular every metric, every pass** — mean 114 vs 126 ms, p95 227 vs 297, p99 323 vs 408, max 437 vs 514 ms. WiFi LAN (separate run): hopssh peak ~423 Mb/s vs Tailscale ~377 Mb/s (~12% higher), TCP cold-start s01 351-400 vs 286-339 Mb/s, no s05 collapse. WiFi jitter inconclusive at 2 passes (run-to-run swing larger than protocol delta — WiFi MAC contention dominates). **Implementation: `cmd/agent/peer_cache.go` (160 LOC, schema v1, 24 h TTL, atomic write, sibling of `relay-state.json`); write hook in `cmd/agent/renew.go::injectPeerEndpoints`; load hook in `cmd/agent/main.go::startMeshInstance` BEFORE `warmTunnel`+`warmPeersFromHeartbeat`. Pure Go, no vendor patch beyond patch 20 (already shipped). Cross-platform — A1 file IO + A2 `net.Dial("udp")` work identically on Darwin/Linux/Windows.** When evaluating future cellular perf changes, this run (`results/perf-20260425-003810/`) is the post-A1 baseline.
...
- **TunMTU 2800 KILLS cellular throughput by 6-7x; 1280 breaks Screen-Sharing HP; 1420 is the sweet spot (final v0.10.9).** Two-axis tradeoff that took bisection. The `nebulacfg.TunMTU = 2800` constant was tuned for WiFi LAN ("halves packet count per screen frame, 13.7ms vs 16.2ms"). On cellular: 2800-byte Nebula packets fragment at the carrier's ~1500 MTU; one fragment loss = whole packet retransmit; cellular has non-trivial loss; throughput collapses. v0.10.8 lowered to 1280 (Tailscale/WireGuard's default) → cellular DL jumped 8 → 52 Mb/s (matches Tailscale). **But 1280 broke macOS Screen Sharing HP entirely** on raw userspace utun: black screen, no warning. avconferenced uses MTU as a heuristic for whether HP is viable; without `IS_VPN` xflag/NE agents (which require NEPacketTunnelProvider — Apple Dev ID + signed bundle), low MTU = HP declines. Empirical bisection on real cellular (2026-04-21, Mac mini ↔ MBP, Yettel BG, direct-P2P verified):

  | MTU | DL (Mb/s) | UL (Mb/s) | DL retransmits | HP screen-share |
  |---|---|---|---|---|
  | 2800 | 7.8 | 1.5 | 191 | warning+retry, works |
  | 1440 | 42 | 6.8 | 1052 | works on first try |
  | **1420** | **49** | **8.3** | **321** | **warning+retry, works ✓** |
  | 1380 | 41 | 6.8 | 1026 | login screen freezes |
  | 1280 | 52 | 9.0 | 9199 | black screen, no warning |

  **1420 wins** on three axes: throughput within cellular variance of Tailscale (49 vs 51), 28× fewer retransmits than 1280 (321 vs 9199 — closer to the path's true capacity), AND stays above the avconferenced MTU heuristic for HP (the threshold sits between 1380 and 1420). Outer-packet math: 1420 + 32 (Nebula) + 28 (UDP/IP) = 1480, fits inside 1500-byte Internet MTU with 20-byte slack for carrier overhead. **Lesson: MTU isn't a one-axis optimization** — fragmentation, retransmits, and macOS HP heuristics interact, and the right value is path-class-AND-application dependent. Long-term real fix for HP is `NEPacketTunnelProvider` so avconferenced gets explicit VPN classification independent of MTU; per-network MTU knob would let users tune (e.g. WiFi-only nets can go higher).
