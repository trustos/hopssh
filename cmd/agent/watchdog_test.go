...
// TestWatchdog_RunServeWiringSourceScan asserts that runServe wires
// inst.restartFn for BOTH boot-time enrollments AND the runtime
// connectFn path. Without this, the watchdog can't auto-recover.
func TestWatchdog_RunServeWiringSourceScan(t *testing.T) {
	...
	body := string(src)
	count := strings.Count(body, "inst.restartFn = func() error { return connectFn(instName) }")
	if count < 2 {
		t.Errorf("inst.restartFn wiring appears %d times in main.go, want >= 2 (boot path + connectFn path)", count)
	}
	if !strings.Contains(body, "var connectFn func(name string) error") {
		t.Error("main.go must forward-declare connectFn as `var` so restartFn closures can reference it")
	}
}
