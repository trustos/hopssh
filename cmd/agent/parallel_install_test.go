...
func TestPortConflictMessageHasReset(t *testing.T) {
	// We can't reach the connectFn closure from outside main.go; assert
	// on the source text instead — same tripwire pattern used by the
	// renew_reload_source_test.go file.
	src, err := os.ReadFile("main.go")
	if err != nil {
		t.Fatalf("read main.go: %v", err)
	}
	if !strings.Contains(string(src), "Settings → Danger zone → Reset") {
		t.Error("connectFn retry-loop error must mention 'Settings → Danger zone → Reset' to point users at cleanup")
	}
	if !strings.Contains(string(src), "another hop-agent on this Mac") {
		t.Error("connectFn retry-loop error must explain port-conflict in user terms")
	}
}
