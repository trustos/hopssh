...
//
// Phase NN: post-extraction the localAPIServer holds a *Client instead
// of separate registry + connectFn fields. We construct a Client with
// connect/disconnect overrides set so tests can intercept those calls
// without spinning up real mesh state.
func newTestLocalAPIServer(t *testing.T, connectFn, disconnectFn func(string) error) (*localAPIServer, http.Handler, string) {
	...
	c := &Client{
		...
		connectOverride:    connectFn,
		disconnectOverride: disconnectFn,
	}
	srv := &localAPIServer{
		client: c,
