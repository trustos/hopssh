...
// RunClientJoin handles `hop client join` — join a mesh network as a
// client device. Exported entry point invoked from cmd/agent/main.go's
// CLI dispatch. Phase NN: previously this was the unexported runClient
// in cmd/agent/client.go; the implementation depends on internal types
// (meshInstance, startMesh, writeFileSecure, etc.) so the entire file
// moved into internal/client and the entry point is exported.
