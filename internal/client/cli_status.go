...
// RunStatus is the exported CLI entry for `hop-agent status`. Phase NN
// extracted status.go from cmd/agent into internal/client because the
// implementation depends on internal types (configDir, Enrollment,
// readServiceStatus, loadPrimaryEnrollment).
...
// readServiceStatus moved to service_status.go in Phase NN — both
// CLI status and local_api.go's status snapshot use it.
