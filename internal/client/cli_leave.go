...
// RunLeave is the exported CLI entrypoint for `hop-agent leave`. Phase NN
// extracted leave.go from cmd/agent into internal/client because the
// implementation depends on the package's internal types.
...
// readDNSConfigForEnrollment moved to dns.go in Phase NN — both
// cli_leave.go and local_api.go's handleLeave call it.
