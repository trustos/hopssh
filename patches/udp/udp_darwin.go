...
func (u *StdConn) ReloadConfig(c *config.C) {
	if rb := c.GetInt("listen.read_buffer", 0); rb > 0 {
		err := u.SetRecvBuffer(rb)
		if err == nil {
			u.l.WithField("size", rb).Info("Set UDP recv buffer")
		} else {
			u.l.WithError(err).Warn("Failed to set UDP recv buffer")
		}
	}
	if wb := c.GetInt("listen.write_buffer", 0); wb > 0 {
		err := u.SetSendBuffer(wb)
		if err == nil {
			u.l.WithField("size", wb).Info("Set UDP send buffer")
		} else {
			u.l.WithError(err).Warn("Failed to set UDP send buffer")
		}
	}
}
