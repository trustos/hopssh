              ...
              <li class="px-4 py-2.5 text-xs">
                <div class="flex items-center justify-between">
                  <div class="flex min-w-0 items-center gap-2">
                    ...
                    {/if}
                    <!-- Phase II.3 (v0.11.3): OS label rendered as
                         plain text, mirroring the dashboard's
                         network-detail OS column at
                         frontend/src/routes/(app)/networks/[id]/
                         +page.svelte:987-994. Hidden for lighthouse
                         rows (they're not a peer machine). -->
                    {#if !p.isLighthouse}
                      {#if p.os === 'darwin'}
                        <span class="shrink-0 text-[10px] text-zinc-500" title="macOS">macOS</span>
                      {:else if p.os === 'linux'}
                        <span class="shrink-0 text-[10px] text-zinc-500" title="Linux">Linux</span>
                      {:else if p.os === 'windows'}
                        <span class="shrink-0 text-[10px] text-zinc-500" title="Windows">Windows</span>
                      {:else if p.os}
                        <span class="shrink-0 text-[10px] text-zinc-500">{p.os}</span>
                      {/if}
                    {/if}
                  </div>
                  <div class="flex shrink-0 items-center gap-3 text-zinc-500">
