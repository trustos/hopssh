...
| N6 | **UPnP-IGD + PCP** | Round out portmap coordinator with UPnP-IGD (legacy SOHO routers) + PCP (RFC 6887, modern routers). Both bolt onto existing `internal/portmap/` Client interface. | NAT-PMP-only covers ~70% of home routers; UPnP catches another 25%, PCP the residual. | M (~800 LOC) | Planned PR 2 (UPnP, ~1.5 d) + PR 4 (PCP bolt-on, ~0.25 d). |
