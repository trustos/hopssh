...
| # | Feature | Description | Why | Size | Viability | Depends on |
|---|---------|-------------|-----|------|-----------|------------|
| 8 | **Granular firewall rules** | Tag-based firewall: assign tags (key:value) to nodes, define rules between tag groups on specific ports/protocols. Compile to Nebula cert groups + firewall config. UI for rule management with preview. Tags, roles, and rules should be reusable across multiple networks in the same account (matches DN's model where a user's rule library applies to all their networks) — schema keys rules to the account/team, not the network, with per-network opt-in. | DN has roles + tags + firewall builder. TS has ACLs (HuJSON — users complain it's complex). ZT has flow rules. Current Nebula config is wide-open allow-all. A visual rule builder beats TS's JSON editing (a common TS complaint). Selfhosters with mixed-trust networks (media server vs NAS vs public-facing) need this. | L | Moderate — Nebula supports group-based firewall natively. Currently certs are issued with hardcoded `groups: ["node"]`. Need: tags table, rule generation, cert re-issuance. Caveat: tag changes require cert renewal (24h cycle, or add immediate renewal endpoint). ~400-600 LOC. | — |
...
| N3 | **P2P on symmetric NAT (asymmetric case)** | Native NAT-PMP (RFC 6886) port mapping in `internal/portmap/` + vendor patch 11. Home-router peer asks UPnP/NAT-PMP-capable router to forward a public UDP port; cellular peer (even behind random-port symmetric CGNAT) reaches it directly. | Most "I'm at home, my laptop is on cellular" pairs were forced to relay. Empirically the dominant failure case for current users. | S (~600 LOC) | **Shipped v0.10.3** Verified on Mac mini ↔ MacBook cellular: 35-43ms direct P2P, down from 100% loss / relay fallback. Per-enrollment unique listen ports (4242, 4243, …) needed too — see N3a. |
| N3a | **Multi-enrollment per-port hotfix** | `Enrollment.ListenPort` field + boot-time `migrateListenPorts` heal. Prevents two Nebula instances racing to bind UDP 4242 (loser would fall back to random port = lighthouse never learns it). | Pre-fix two-network hosts had silently broken P2P for the secondary enrollment. | XS (~150 LOC + tests) | **Shipped v0.10.3** Migration runs once on boot for legacy enrollments. |
| N4 | **P2P on double-symmetric NAT** | Birthday-paradox simultaneous-burst port scanning — 256 source ports × 256 destination probes (Pillar 2 in three-pillar plan). Coordinated via lighthouse `BurstSyncNotification` so both peers burst within ±250 ms. | Both-cellular pairs still fall back to relay even after N3. | M (~400 LOC, vendor patch 12) | Designed in `spike/relay-vs-tailscale-evidence/IMPLEMENTATION-PLAN.md`. ~3.5 days. |
| N5 | **Peer relay (user-contributed)** | Designate any node as a relay via `Node.Capabilities` (`"relay"` value, validated server-side); heartbeat response carries `amRelay` + `relays` lists; agent caches to `<configDir>/<name>/relay-state.json`; `ensureP2PConfig` reads cache and writes `relay.am_relay` + extends `relay.relays`. | Single Oracle-Cloud central relay = high RTT for EU users (200+ ms) and capacity SPOF. | M (~400 LOC + tests) | **Shipped v0.10.5** Capability change takes effect on next restart / cert renewal (24h cycle). Live reload-on-toggle deferred. |
| N6 | **UPnP-IGD** | SSDP discovery (`internal/portmap/upnp_ssdp.go`) → device-description XML walk → SOAP `AddPortMapping` + `GetExternalIPAddress`. Includes 725 (`OnlyPermanentLeasesSupported`) retry-with-lease-0 path and double-NAT detection (rejects private external IPs). | NAT-PMP catches ~30-40% of consumer routers (Apple-style + miniupnpd-based); UPnP catches the much larger ~70-80% (Netgear, ASUS, Linksys, Verizon FiOS, ISP-supplied boxes). | M (~700 LOC + tests) | **Shipped v0.10.4** Verified live on TP-Link (UPnP WANIPConnection:1) — `46.10.240.91:44245` mapped, 2h TTL. 12 hermetic unit tests + 1 build-tag-gated live test. |
| N7 | **PCP (RFC 6887)** | NAT-PMP successor with IPv6 support and explicit nonce-based mapping correlation. Same wire transport as NAT-PMP (UDP 5351); 60-byte MAP request with 12-byte nonce. Bolts onto existing NAT-PMP roundtrip code. | Modern routers (FiOS-issued, newer Linksys) speak PCP only. | S (~250 LOC + 7 tests) | **Shipped v0.10.5** Coordinator now probes NAT-PMP, PCP, and UPnP in parallel — combined coverage ≈100% of port-mapping-capable consumer routers. |
| N8 | **Birthday-paradox port prediction (primitive)** | `internal/burstsock` package: pool of N UDP sockets × K candidate destination ports → ~63% collision/cycle at N=K=256. Pacing-aware to stay under carrier rate-limits. Helpers `RandomPorts` and `HotPortsFirst` for candidate-port generation. | Bidirectional carrier-NAT (both peers cellular) — the residual ~5-10% the asymmetric solution doesn't cover. | S (~400 LOC + 12 tests) | **Shipped v0.10.5** as a primitive only. Full Nebula handshake-manager integration (lighthouse-coordinated burst trigger via new proto message + vendor patch 12) is the follow-up. |
...
| 19 | **IPv6 overlay (v2 cert migration)** | Support IPv6 addresses in the mesh overlay using Nebula v2 certificates. Scope includes the v1→v2 cert cutover, not just IPv6 encoding — extend `internal/pki/pki.go:52-90` `IssueCert` to accept `ips []netip.Prefix` (slice replaces the singular `ip`), flip `TBSCertificate.Version` to v2 (`vendor/github.com/slackhq/nebula/cert/cert_v2.go`), widen `nodes.ip` to support IPv6 CIDRs, extend subnet allocator. A single cert's `Networks` can then carry dual-stack v4+v6, per [the IPv6 blog](https://www.defined.net/blog/year-of-the-ipv6-overlay-network/). | ZeroTier and Tailscale both support IPv6. Nebula v2 certs add this natively. Needed for modern infrastructure. | M | High — cert format, CAPool, and TUN multi-bind already support v2 in the vendored code. Migration strategy: dual-CA via `pki.ca` PEM bundle lets v1 and v2 nodes coexist during rollout — both CAs in the trust bundle, new enrollments issue v2, old v1 certs renew to v2 on their next 24h cycle. No flag day required. | — |
| 19a | **CA rotation with overlap** | Rotate a network's CA with a configurable overlap period. Control plane adds a new CA, issues new certs signed by it, keeps the old CA in the trust bundle until all nodes renew, then retires the old CA. Dashboard UI to initiate + monitor rotation progress (count of nodes still on the old CA). | Security hygiene — network CAs sign node certs indefinitely; they should be rotatable without downtime if a key is suspected compromised or on a routine schedule. DN doesn't expose this in their UI either — modest differentiator. Same machinery also underlies #19's v1→v2 transition, so infrastructure lands once. | M | Moderate — Nebula's `pki.ca` field already accepts a PEM bundle of multiple CAs (`vendor/github.com/slackhq/nebula/cert/`). Server changes: `network_cas` table (active + trusted list), cert issuance uses active CA, agent config emits the full trust bundle, renew endpoint knows which CA to sign under, dashboard shows per-network rotation state. ~400-600 LOC. | #19 (shared cert-format machinery) |
| 19b | **Zero-downtime CIDR migration** | Change a network's subnet (e.g., outgrow `/24` → `/22`, or renumber from `10.42.0.0/16` to `10.100.0.0/16`) without downtime. Dual-address each node's cert with both the old and new IP during the cutover window; flip lighthouse + DNS to the new range; drain and retire old range when all renewals complete. | Called out as a primary use case in [DN's multi-network blog](https://www.defined.net/blog/multiple-networks/). Selfhosters hit this when a homelab grows past 254 nodes, or when IP ranges clash with a corporate LAN subnet they later bridge to. Today our only workaround is "create a new network and re-enroll everything." | L | Depends on #19's `Networks []netip.Prefix` widening — once `IssueCert` accepts multiple prefixes, dual-addressing a node during migration is a matter of allocation state (old IP + new IP held simultaneously) + renewal flow that stops issuing the old IP once drain completes. ~500-800 LOC on top of #19. | #19 |
...
| 29 | **Multi-network per agent** ✅ shipped v0.10.0 | A single agent joins multiple networks simultaneously (one physical machine → N mesh memberships). Architecture matches Defined Networking's canonical approach ([DN multi-network guide](https://www.defined.net/blog/multiple-networks/)): N Nebula instances per N networks, each with its own CA/cert/tun, because per-network CA isolation is part of Nebula's trust model and cannot be collapsed to a single-instance multi-CA setup (upstream confirmed in slackhq/nebula#235, #251, #306). | ZeroTier's core differentiator; NetBird supports too. Selfhosters running homelab + work mesh side-by-side need this. Unlocks "one daemon, many overlays" which is table stakes for power users and a gate for any team-of-teams workflow. | L | Shipped. Per-enrollment subdir layout (`<configDir>/<name>/`) with auto-migration of pre-v0.10 flat installs, per-instance Nebula + heartbeat + cert renewal + DNS, `hop-agent leave/status --network`. Phase E verified on Mac mini + MacBook + Ubuntu 25.10 + Windows 11 ARM64 (evidence at `spike/multi-network-evidence/`). Plan document: `docs/multi-network-per-agent-execution.md`. Known minor tech debt: `go-metrics` uses static counter names, so N instances share counters — per-instance `logrus.WithField` disambiguates logs but metrics aggregate across instances. Fix is a per-instance metric prefix; not urgent. | — |
