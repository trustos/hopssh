...
| 29 | **Multi-network per agent** 🎯 next major | A single agent joins multiple networks simultaneously (one physical machine → N mesh memberships). | ZeroTier's core differentiator; NetBird supports too. Selfhosters running homelab + work mesh side-by-side need this. Unlocks "one daemon, many overlays" which is table stakes for power users and a gate for any team-of-teams workflow. | L | Agent architecture is singleton today (one config dir, one Nebula instance, one bearer token, one heartbeat goroutine). Refactor target: `map[networkID]meshInstance` keyed by enrollment. Invariants already pinned in recent features (one heartbeat POST = one nodeID = one network). Plan in `docs/multi-network-per-agent-plan.md`. 4-6 weeks. | — |
