...
| N5 | **Peer relay (user-contributed)** | Designate any node as a relay via `Node.Capabilities`; others auto-discover via control plane → `nebula.yaml` `relay.relays` list (Pillar 3). | Single Oracle-Cloud central relay = high RTT for EU users (200+ ms) and capacity SPOF. | M (~600 LOC + dashboard toggle) | Capability infrastructure exists; Nebula `am_relay` already independent of `am_lighthouse` (verified). ~3 days. |
...
| N7 | **PCP (RFC 6887)** | NAT-PMP successor with IPv6 support and explicit nonce-based mapping correlation. Same wire transport as NAT-PMP (UDP 5351); 60-byte MAP request with 12-byte nonce. | Modern routers (FiOS-issued, newer Linksys) speak PCP only. | S (~180 LOC, bolt-on to existing portmap.Client interface) | Planned PR 4 (~0.25 d, bolts onto NAT-PMP roundtrip code). |
