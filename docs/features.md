...
- **NAT-PMP + UPnP-IGD port mapping (v0.10.3+ NAT-PMP, v0.10.4+ UPnP)** — On startup, each enrollment asks the local home router to forward a public UDP port to its Nebula listen port. Tries NAT-PMP (RFC 6886) and UPnP-IGD in parallel; first successful response wins. Coverage: NAT-PMP catches Apple-AirPort-style and modern TP-Link/miniupnpd routers; UPnP catches the much larger pool of consumer routers (Netgear, ASUS, Linksys, Verizon FiOS, ISP-supplied boxes — most home networks deployed since 2005). The resulting `public_ip:public_port` is injected into the lighthouse's `advertise_addrs` (vendor patch 11). Peers — including those behind random-port symmetric CGNAT (cellular) — reach this node directly instead of falling back to relay. Verified end-to-end: cellular-CGNAT MacBook ↔ home-router Mac mini achieves 35-43 ms direct P2P RTT (was 200+ ms via relay). PCP (RFC 6887) coming in v0.10.5+.
