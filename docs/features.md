...
- **Persistent peer-endpoint cache (v0.10.20, Phase A1)** — Each enrollment writes the most recent peer endpoints learned via heartbeat to disk (`<configDir>/<instance>/peers.json`, schema v1, 24 h TTL, atomic write). Loaded at agent startup BEFORE the first handshake fires, so the first attempt goes direct instead of racing relay-vs-direct. Eliminated the cellular cold-start `s05` cwnd collapse (pre-A1: 87.6 → 22.0 → 26.1 → 51.6 Mb/s; post-A1: 40-50 Mb/s flat). Combined with a 1-byte UDP-hello probe (Phase A2) that opens cellular CGNAT outbound flow + populates kernel ARP/route cache.
- **Per-peer EWMA RTT observability (v0.10.21, Phase B-lite)** — `cmd/agent/path_quality.go` probes each direct peer's mesh listener (TCP-connect to `:41820`) every 10 s and tracks an exponentially-weighted moving average (α=0.3). Surfaces in `PeerDetail.RTTms` via the existing heartbeat → server → dashboard pipeline. Logs a single warning when 3 consecutive samples land >50 ms above the EWMA baseline. Pure observability — no auto-swap of `CurrentRemote`. Cost: one TCP-SYN per direct peer per 10 s.
- **HTTPS-distributed peer endpoints (v0.10.22, Phase G)** — Agent reports its own observed reachable endpoints (NAT-PMP public + physical-egress-interface IP paired with listen port) in every heartbeat POST. Server caches per-node with 15-min TTL and merges them into `peerEndpoints` responses to OTHER agents. Independent of UDP-to-lighthouse propagation — works even when carrier blocks UDP entirely. Closes the "cellular peer goes silent at idle" gap. Verified by `scripts/cellular_idle_test.sh`: 17 endpoint pushes during 17-min idle window; mesh recovers without manual restart. Restricted to physical-egress interface in v0.10.25 to avoid advertising overlay/VPN-tap addresses (ZeroTier, Tailscale, Cloudflare WARP).
- **Cert-renewal lifecycle hardening (v0.10.26)** — `reloadNebula` restructured to try-then-swap (only kill watcher + old svc AFTER new svc starts successfully) plus retry-with-backoff goroutine (5 s → 30 m × 4 cap, max 10 attempts) so transient failures self-recover without operator intervention. `watchNetworkChanges` gained `defer recover()`, startup log, and periodic alive log every ~16 min for fast post-deploy verification. Boot-time duplicate-listen-port self-heal added (`HealDuplicateListenPorts`) so any future regression OR manual edit producing duplicates recovers on next process start. See `docs/sleep-wake-plan.md` for full diagnosis of the cascading bug this addressed.
...
- **"Hide hopssh from the Dock" toggle (Phase N)** — Settings → Preferences toggle that flips the macOS activation policy between `.regular` (default — Dock icon, Cmd-Tab entry, full app behavior) and `.accessory` (menubar-only — no Dock icon, hidden from Cmd-Tab, NSStatusItem still rendered). Persisted to `~/Library/Application Support/hopssh/desktop-prefs.json`. Demotion fires on `WindowEvent::CloseRequested` (so the user finishes their current interaction before the Dock icon disappears); restoration fires on `RunEvent::Reopen` (Dock-click while running) and `show_main_window` (tray "Show hopssh" menu item). Apple HIG explicitly endorses this for VPN/network utilities; the runtime-toggle pattern matches Slack, 1Password 7, and Hammerspoon.
...
- **UI honesty + watchdog (Phase P, v0.10.79)** — `enrollmentStatus.Connected` derived from `certValid && (peers > 0 || recentHeartbeat)` instead of the prior `ctrl != nil`. Renewal-goroutine silent-death watchdog stamps `lastRenewalActivityAt` at every observable point and writes a forensic dump if silent past `renewCertDuration / 4`.
- **Sleep-wake survival (Phase S, v0.10.82)** — Cert renewal loop uses `time.NewTicker(60s)` re-reading wall-clock NotAfter on every tick instead of `time.After(longSleep)`. Survives macOS Clamshell-Sleep / DarkWake cycles where Go's monotonic clock freezes. Tested via SIGSTOP/SIGCONT sub-process pattern.
- **Nodes table polish (Phase T, v0.10.83)** — Dashboard's Nodes table gets per-column toggles (show/hide OS / Client / RTT / etc), and the "Handshake" column (always-blank for Nebula's vendored runtime) is replaced with the agent-measured EWMA RTT.
- **Member-role enrollment fix (Phase V, v0.10.85)** — Member users (non-admin) can now enroll devices into networks they've been invited to. Pre-fix: `POST /api/device/authorize` returned HTTP 404 because `CanAccessNetwork` was owner-only.
- **State-endpoint TCP probe (Phase X, v0.10.89)** — Desktop client validates the cached system-mode endpoint via 200ms TCP probe before reuse. Recovers automatically from `launchctl kickstart` / dev-deploy / daemon flap that rotates the loopback port + token. SSE-failure path proactively triggers `retry_attach_system_agent` so the .app self-heals in ~5s without user action.
- **Autostart on macOS login (Phase Y, v0.10.90)** — `tauri-plugin-autostart` writes `~/Library/LaunchAgents/com.hopssh.desktop.plist` with `--start-minimized`. Settings → Preferences toggle "Open hopssh on login". First system-mode opt-in flips this ON automatically (with `start_at_login_explicit` sentinel preserving subsequent user overrides).
- **System-mode chown self-heal (Phase Z, v0.10.91)** — System-mode mirror files at `~/Library/Application Support/hopssh/system-local-api-{port,token}` are re-chowned every 30s via `runMirrorChownSelfHeal` if they end up root-owned (boot-before-login race where `/dev/console` is root-owned). The .app reads them via the user; this self-heal makes the boot-time race structurally recoverable.
- **Uninstall hygiene (Phase AA, v0.10.92)** — Settings → Danger zone → Uninstall now also removes the Phase Y autostart LaunchAgent + flushes SystemUIServer + ControlCenter to clear NSStatusItem ghosts. `tauri-plugin-autostart::disable()` is called BEFORE the privileged uninstall script (so launchd unloads the plist before the file is deleted).
- **UX copy audit (Phase BB, v0.10.93)** — 28 protocol-jargon strings rewritten across Settings/Onboarding/Connected/Disconnected.svelte. "mesh"→"network", "data-plane"→"connection", "hop-agent"→"command-line tool". "Hide hopssh from the Dock" → affirmative "Show hopssh in Dock" (Apple HIG; Slack/1Password/Discord pattern), with the UI inverting the underlying `hide_from_dock` pref via a `showInDock` `$derived`. Tripwire test source-scans every Svelte file's body for forbidden tokens.
- **`/version` reports `max(current, fetched)` (Phase CC, v0.10.95)** — Smooths the version-check window right after a tag bump where GitHub Releases hasn't published yet but the new container is already running. The dashboard's update banner no longer flashes "Update available: vN-1" while running vN.
- **Watcher-watchdog (Phase DD, v0.10.96)** — Third independent watchdog in the agent (alongside renewal + data-plane). `runWatcherWatchdog` stamps `lastWatcherActivityAt` at the top of every `watchNetworkChanges` tick body; fires CRITICAL + forensic dump + `restartFn` after 3 min of silence. Catches vendor-Nebula deadlocks in `RebindUDPServer` / `CloseAllTunnels` (`defer recover()` only catches panics, not deadlocks). Hard 5s timeouts wrap the two vendor calls so the watcher self-recovers without needing the watchdog to fire in the common case.
- **Polish bundle (Phase EE, v0.10.97)** — `desktop-prefs.json` corruption logging (was silently eating JSON parse failures); device-identity hostname affordance in Connected hero (via new `LocalStatus.Hostname` field); disabled-button tooltips on the connect/disconnect button; onboarding error specificity (no more generic "Couldn't connect" fallback when agent provides a real reason); post-uninstall blocking view + 5s auto-quit countdown.
- **Activity tab (Phase FF, v0.10.98)** — New top-level Activity tab in the desktop client surfaces the buffered SSE event stream (capped at 200) with filters: All / Connection / Networks / Recovery. Severity dots (info/warn/error) for at-a-glance scan; plain-English summaries; footer points to Settings → About → View agent logs for forensic depth.
- **Diagnostics tools (Phase GG, v0.10.99)** — Settings → About gets two new buttons. "View agent logs" opens Console.app pointing at `/var/log/hop-agent.log` (system mode) or just opens Console.app (bundled mode). "Copy diagnostic info" formats version + commit + os/arch + hostname + per-enrollment summary into a paste-friendly plain-text block on the clipboard for support tickets.
- **DNS records read-only view (Phase HH, v0.11.0)** — New "DNS records" section in Connected.svelte flattens `peer.dnsHostname` + `peer.customDnsNames` (already in agent's heartbeat-fed peer data) into a sorted list with "custom" badges. Read-only — "Manage in dashboard ↗" link bridges to the web UI for create/delete (write needs server-side admin auth refactor).
- **In-app Terminal via dashboard webview (Phase II.3, v0.11.3)** — Per-peer "Terminal" button opens the dashboard's xterm.js terminal page (`/terminal/{networkId}/{nodeId}?h=<hostname>`) in a NEW Tauri webview window. Cookie storage shared across Tauri webviews — first open prompts dashboard login; subsequent opens reuse the persisted session. New Tauri command `open_terminal_webview` (cross-platform, UUID-shape input validation, dedup by label per peer). Server-side change: heartbeat response now carries top-level `networkId` + per-peer `nodeId` (both `omitempty`, backwards-compatible).
- **Lighthouse special-case + OS icons (Phase II.2 → II.4, v0.11.2 → v0.11.4)** — Peers list visually identifies infrastructure (lighthouse) vs operable peer machines. Lighthouse rows show the label "Lighthouse" + no Terminal button (no shell to attach to). Each peer row shows an OS brand-mark icon (Apple / Tux silhouette / Windows panes) with hover tooltip showing the OS name. Dashboard uses shadcn Tooltip primitive (already-installed: bits-ui + lucide-svelte + the existing tooltip component). Desktop client uses the same SVG paths inlined with native HTML `title=` attribute (no shadcn dep on the desktop side). Saves ~30-33px column width per row vs the prior text labels; matches the device-list pattern of Tailscale / Slack / 1Password.
