...
Scope this covers: macOS 15.x on Apple Silicon (same-LAN WiFi + iPhone hotspot cellular CGNAT, two-node topology, kernel-TUN mode) + Linux VM sleep-simulation via SIGSTOP + Windows 11 ARM64 VM sleep-simulation via NtSuspendProcess. Behavior on **bare-metal Linux suspend, bare-metal Windows suspend, SSID-roam mid-sleep (WiFi↔cellular switch on wake), multi-hop, mobile battery** is **not yet measured** — those are the dimensions where competitor VPNs have long-open issues (Tailscale #17736, #10688, #1554; ZeroTier #2026, #2545; NetBird #2454), and we have no data either way.
...
| 2 | **Sleep/wake resilience** | ✅ macOS LAN + cellular PASS; Linux VM SIGSTOP PASS; Windows 11 ARM64 VM NtSuspendProcess PASS — `sleep/wake detected (tick gap 2m35s)` log + 1s peer recovery. Linux DNS + Windows NRPT bugs discovered and **fixed in v0.9.9**. Bare-metal Linux/Windows suspend and SSID-roam-mid-sleep still unmeasured. Evidence: `spike/sleep-wake-evidence/RESULTS.md`. | macOS + Linux + Windows done; bare-metal needs hardware | See `#sleep-wake-recovery` section above. |
